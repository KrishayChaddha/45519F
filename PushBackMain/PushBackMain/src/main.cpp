...
void pre_auton() {
  ...
  verticalOdom.setPosition(0, deg);

  Piston.set(false); // pistons extended initially
  ...
  while(!auto_started){
    ...
    switch(current_auton_selection){
      case 0:
        Brain.Screen.printAt(5, 140, "Game Autonomous");
        break;
      case 1:
        Brain.Screen.printAt(5, 140, "Skills Autonomous");
        break;
      case 2:
...
void usercontrol(void) {
  ...
  double outtakeVelocity = 85;
  bool movingUp = false; // tracks the direction
  ...
  while (1) {
    ...
    if(Controller1.ButtonX.pressing() && outtakeState != UP && outtakeState != MOVING) {
        outtakeState = MOVING;
        movingUp = true;
        outtakeTimer = Brain.timer(msec);
        outtake.spin(reverse, outtakeVelocity, pct);  // up
    }

    // Lower B only if not down
    if(Controller1.ButtonB.pressing() && outtakeState != DOWN && outtakeState != MOVING) {
        outtakeState = MOVING;
        movingUp = false;
        outtakeTimer = Brain.timer(msec);
        outtake.spin(forward, outtakeVelocity, pct);  // down
    }

    // Stop outtake after 600 ms when funnel is up, 1000 ms when funnel is down
    if(outtakeState == MOVING) {
      if(pistonState == EXTENDED && Brain.timer(msec) - outtakeTimer >= 600) {
          outtake.stop();
      } else if(pistonState == RETRACTED && Brain.timer(msec) - outtakeTimer >= 1000) {
          outtake.stop();
      }
