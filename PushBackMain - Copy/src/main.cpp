...
void usercontrol(void) {
  ...
  while (1) {
    ...
    if(pistonState == RETRACTED) {
      outtake.setVelocity(80, pct);
    } else {
      outtake.setVelocity(50, pct);
    }
    ...
    if(Controller1.ButtonX.pressing() && outtakeState != UP && outtakeState != MOVING) {
        outtakeState = MOVING;
        if (pistonState == RETRACTED) {
            outtake.spinFor(reverse, 366, deg, false); //up
        } else {
            outtake.spinFor(reverse, 390, deg, false); //up
        }
        outtakeState = UP;
    }
    ...
    if(Controller1.ButtonB.pressing() && outtakeState != DOWN && outtakeState != MOVING) {
        outtakeState = MOVING;
        if (pistonState == RETRACTED) {
            outtake.spinFor(forward, 366, deg, false); //down
        } else {
            outtake.spinFor(forward, 390, deg, false); //down
        }
        outtakeState = DOWN;
