...
void default_constants() {
  // P, I, D, and Start I
  chassis.pid_drive_constants_set(11.0, 5.0, 37.0); // Fwd/rev constants, used for odom and non odom motions
  chassis.pid_heading_constants_set(11.0, 5.0, 37.0); // Holds the robot straight while going forward without odom
  chassis.pid_turn_constants_set(1.05, 0.01, 4.5, 10.0); // Turn constants
...
void left_side() {
  ...
  chassis.pid_turn_set(-224_deg, TURN_SPEED);
  chassis.pid_wait();
...
void right_side() {
  ...
  intake.move(127);
  chassis.pid_drive_set(27.8_in, 40);
  chassis.pid_wait();
  matchload.set(true);
  pros::delay(200);

  chassis.pid_drive_set(1.2_in, 100);
  chassis.pid_wait();
  intake.move(0);
  matchload.set(false);
  ...
  chassis.pid_drive_set(-48.5_in, 127);
  chassis.pid_wait(); 
  pros::delay(600); 
  ...
  chassis.pid_drive_set(13.6_in, 100);
  chassis.pid_wait();
  pros::delay(60);
  ...
  intake.move(127);
  chassis.pid_drive_set(-26_in, 117);  
  chassis.pid_wait();

  hood.set(true);
  outtake.move_relative(720, 100);
  pros::delay(700); 
...
void right_side_extra() {
  ...
  chassis.pid_drive_set(-25.7_in, DRIVE_SPEED);
  chassis.pid_wait();
  pros::delay(200);
  ...
  chassis.pid_wait();
  pros::delay(200);
  chassis.pid_turn_set(217_deg, TURN_SPEED);
  chassis.pid_wait();
  pros::delay(200);

  intake.move(127);
  chassis.pid_drive_set(35_in, 30);
  chassis.pid_wait();
  pros::delay(200);
...
void solo_auton() {
  ...
  // Drive first 17 inches
  chassis.pid_drive_set(19_in, 127);
  chassis.pid_wait();
  pros::delay(10);


  // Deploy matchload at ~17 inches
  matchload.set(true);
  pros::delay(350);                 // Optional delay to fully deploy

  // Drive remaining 11 inches
  chassis.pid_drive_set(13_in, 127);
  chassis.pid_wait();
  pros::delay(400);

  // Retract matchload and stop intake
  matchload.set(false);
  intake.move(0);
  ...
  chassis.pid_drive_set(51_in, 100); 
  chassis.pid_wait();

  // Stop intake and retract matchload
  intake.move(0);                    
  matchload.set(false);              
...
void skills_auton() {
  ...
  chassis.pid_drive_set(31.0_in, 70);
  chassis.pid_wait();
  ...
  chassis.pid_drive_set(90_in, 80);
  chassis.pid_wait();
  ...
  intake.move(127);
  chassis.pid_drive_set(-11.0_in, 70);
  chassis.pid_wait();
  intake.move(0);
  pros::delay(200);
  ...
  chassis.pid_drive_set(7.15_in, 70);
  chassis.pid_wait();

  chassis.pid_turn_set(180_deg, TURN_SPEED);
  chassis.pid_wait();

  chassis.pid_drive_set(93_in, 70);
  chassis.pid_wait();
  ...
  chassis.pid_drive_set(9.7_in, 70);
  chassis.pid_wait();
  ...
  chassis.pid_drive_set(19_in, 70);
  chassis.pid_wait();
  ...
  chassis.pid_wait();
  intake.move(127);
  chassis.pid_drive_set(37.5_in, 100);
  chassis.pid_wait(); 
}